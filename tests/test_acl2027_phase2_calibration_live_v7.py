...
from scripts.audit_acl2027_phase2_calibration_v7 import build_audit, close_authorization
...
from scripts.run_acl2027_phase2_calibration_token_plan_live_v7 import (
    AUTH,
    REGISTRY,
    HardStop,
...
def test_authorization_is_exactly_bound_and_calibration_only(tmp_path: Path) -> None:
    auth = load(AUTH)
    ...
    auth_copy = tmp_path / "authorization.json"
    auth_copy.write_bytes(AUTH.read_bytes())
    registry = tmp_path / "registry.json"
    write(registry, {"schema_version": 7, "authorizations": {auth["authorization_id"]: {"path": "authorization.json", "sha256": sha256_file(auth_copy)}}})
    assert validate_live_authorization(registry, auth["authorization_id"])["authorization_id"] == auth["authorization_id"]
...
def test_mock_60_call_paced_audit_and_irrevocable_closure(tmp_path: Path) -> None:
    ...
    write(registry, {"schema_version": 7, "authorizations": {auth["authorization_id"]: {"path": "../authorization.json", "sha256": sha256_file(auth_path)}}})
...
def test_unknown_terminal_usage_is_not_reported_as_exact_zero(tmp_path: Path) -> None:
    artifact = tmp_path / "artifact"
    artifact.mkdir()
    auth_path = tmp_path / "authorization.json"
    auth_path.write_bytes(AUTH.read_bytes())
    ...
    assert audit["lower_bound_known_usage_cost_cny"] == 0.0
