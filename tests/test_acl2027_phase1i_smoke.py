"""Offline contracts for the authorized Phase 1I repaired live smoke."""
...
from scripts.run_acl2027_phase1i_smoke import (
    DEFAULT_CONFIG,
    DEFAULT_STATE,
    SmokeError,
    execute_smoke,
...
def write_state(path: Path, paid: bool = True) -> None:
    ...
    state["execution_policy"]["reason"] = "User authorized exactly two repaired calls; OfficeQA 24 and SpreadsheetBench 40 remain closed."
    state["current_phase"]["id"] = "1I"
...
def test_dry_run_is_bound_to_phase1h_two_request_hashes():
    ...
    assert [row["request_hash"] for row in result["requests"]] == [
        "47b00e72aad2925d904f1ae66f50a6dc19ac3a0ec50241550d6f6c2e346a8c39",
        "f57b377fe13233f3177f84f3b98f07dd3abfb1cdd3dd2b59542926a7a92ff4d2",
    ]
...
def test_fake_live_executes_two_calls_and_passes_both_task_gates(tmp_path):
    ...
    assert result["decision"] == "repaired_smoke_passed_batch_still_closed"
...
def test_task_invalid_first_response_is_charged_and_second_call_still_audited(tmp_path):
    ...
    output = tmp_path / "artifact"
    result = execute_smoke(state_path=state_path, output_dir=output, live=True, client=client)
    assert result["decision"] == "repaired_smoke_failed_batch_closed"
    assert result["provider_attempts"] == 2
