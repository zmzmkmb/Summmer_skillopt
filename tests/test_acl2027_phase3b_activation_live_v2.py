import json
import pytest
from scripts import analyze_acl2027_phase3b_activation_live_v2 as analysis
from scripts import run_acl2027_phase3b_activation_live_v2 as live
...
class MockProvider:
    ...
    def __call__(self, body):
        ...
        return {"content": json.dumps({"declared_skill_applicability": selected != "none", "selected_skill_id": selected, "intermediate_operation": "retrieve", "final_answer": "mock"}), "usage": {"input_tokens":10,"output_tokens":4,"total_tokens":14}, "request_id": f"mock-phase3b-{self.calls}"}

def patch_artifact(tmp_path, monkeypatch):
    artifact = tmp_path / "phase3b-v2"
...
def test_phase3b_v2_preflight_binds_exact_plan(monkeypatch):
    ...
    auth = live.authorization_template("mock-phase3b-secret"); assert auth["authorized_stage"] == "activation_pilot" and auth["stage_cost_ceiling_cny"] == 3.0 and auth["cumulative_cost_ceiling_cny"] == 11.0 and auth["formal_scaling_allowed"] is False

def test_phase3b_v2_terminal_error_is_not_resumable(tmp_path, monkeypatch):
...
def test_phase3b_v2_receipt_rejects_binding_drift(tmp_path, monkeypatch):
    receipt = live.load(live.RECEIPT); receipt["bindings"]["phase3b_schedule_sha256"] = "0"*64; path = tmp_path/"receipt.json"; live.write(path,receipt); monkeypatch.setattr(live,"RECEIPT",path)
    with pytest.raises(live.HardStop,match="receipt binding drift"): live.preflight()
