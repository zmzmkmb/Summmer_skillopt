...
| NSGA-II（正式实验） | pop=30, gen=15, mutation_p=0.10, crossover_p=0.90 |
| 权重 | [0.4, 0.3, 0.2, 0.1] |
...
| **MOAR** | **70.67% +/- 0.29%** | 70.50%, 71.00%, 70.50% | 5.0 | ~1918 | ~300ms (pop=30,gen=15) |
...
> **配置说明**: 本验证使用更重的 NSGA-II 配置（pop=50, gen=30, ~2000ms/query），
> 与正式准确率实验的配置（pop=30, gen=15, ~300ms/query）不同。
> 目的是在最大搜索预算下评估 MOAR 与穷举最优解的差距上限。
...
3. **预算约束严格遵守**: 无预算违规
4. **检索延迟可接受**: ~300ms/query (pop=30, gen=15，正式实验配置)；更重配置 (pop=50, gen=30) 约 2000ms/query（Exact 验证使用）

### 当前 BM25/Greedy 更高的原因

1. **小规模规则库**: 仅 8 条动态规则，简单的词法匹配（BM25）已足够有效
2. **冷启动**: MOAR 使用 0 效用值，无法利用历史反馈（与 Greedy-Utility 不同）
3. **NSGA-II 开销**: 在 218 种候选子集的可穷举空间中，精确搜索比近似搜索更快且保证标量最优
