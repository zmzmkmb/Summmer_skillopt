...
1. **两套度量差距均较大**：这不是 NSGA-II 搜索失败，而是 NSGA-II 和 Exact 做了**不同的事**：
   - Exact：穷举所有合法组合，直接取 M_moar（或 M_exact）标量公式的最大值——单目标优化
   - MOAR：进化多目标 Pareto 前沿（4 个独立目标），再用加权和从前沿中选一个——多目标优化
   
2. **M_moar 是事后标量化**：M_moar 把四个目标加权合并成一个数，但 NSGA-II 并不直接优化这个数。它分别优化四个目标，维持 Pareto 前沿的多样性。加权和只是最终选方案时的后处理。

3. **Exact 更快**：n=8 时穷举 256 种组合仅需 ~20ms，比 MOAR 的 ~2000ms 更快。当规则数增至 16+ 时，穷举 O(2^n) 不可行，MOAR 将体现效率优势。
...
> 在当前 8 条动态规则的可穷举场景下，MOAR 的多目标 Pareto 优化路径与 Exact 的单目标标量最大化路径在 M_moar 上平均相差 ~0.19。这不是搜索失败——NSGA-II 不直接最大化 M_moar 标量值，而是通过四维 Pareto 前沿 + 加权和选解。两种方法"得到不同的解"不等同于"一种方法错误"。在 n=8 时穷举是最佳选择；规则数增至 16+ 时 MOAR 将体现搜索效率优势。
