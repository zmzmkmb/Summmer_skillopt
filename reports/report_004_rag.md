...
|------|------|:--:|:--:|:--:|------|
| 🥇 | **#004 RAG** | 0.7250 | **0** | **31M** | ✅ 生产推荐 |
| 🥈 | #002 gated | 0.7157 | 0 | 63M | ✅ 保守选择 |
| 🥉 | #003 退火 | 0.7214 | -0.0107 | 90M | ⚠️ 实验性 |
| 4 | #001 force-accept | 0.7400 | -0.0114 | 32M | ❌ 假停滞 |

> 注：#001 峰值最高但不可复现——部分来自 force-accept slow update 的"脏金"
...
1. **RAG 是四轮中最佳配置** — 零退化 + 0.7250 test + 31M token，性价比最高
2. **RAG 比退火更有效** — 每 query 精准选规则胜过全局随机探索
3. **Prompt 减半** — 从 10,000 → 2,000 字符/query，token 消耗减半
4. **和 #002 (gated) 正交** — RAG + gated slow update 的组合是当前最优
5. **为后续四门架构打下基础** — RuleMemory + 规则级操作已经就位，输入门/遗忘门/输出门可以直接在此基础上构建
