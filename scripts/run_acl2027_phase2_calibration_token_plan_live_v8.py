#!/usr/bin/env python3
"""Live-only orchestration for a separately authorized Phase 2 v8 calibration."""
...
from scripts.audit_acl2027_phase2_calibration_v8 import close_authorization
...
AUTH = ROOT / "configs/acl2027/phase2_calibration_live_authorization_v8.json"
ARTIFACT = ROOT / "artifacts/acl2027_phase2_calibration_live_v8"
...
AUDIT_SOURCE = ROOT / "scripts/audit_acl2027_phase2_calibration_v8.py"
...
def validate_live_authorization(registry_path: Path, authorization_id: str) -> dict[str, Any]:
    ...
    if authorization_id not in registry:
        raise HardStop("v8 live authorization missing or closed")
    ...
    if auth.get("live_execution_bindings") != expected:
        raise HardStop("v8 live execution source binding drift")
    if auth.get("user_authorization", {}).get("scope") != "phase2_v8_calibration_only":
        raise HardStop("v8 user authorization scope drift")
    if auth.get("user_authorization", {}).get("not_inherited_by_later_stages") is not True:
        raise HardStop("v8 later-stage prohibition drift")
    key = os.environ.get("DASHSCOPE_API_KEY")
    if not key or auth.get("credential_sha256") != hashlib.sha256(key.encode("utf-8")).hexdigest():
        raise HardStop("v8 credential binding drift")
    return auth
...
class PersistentRequestStartPacer:
    ...
    def __call__(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(starts) != len(records):
            raise HardStop("v8 ambiguous started request; automatic retry forbidden", records)
...
def execute_live(authorization_id: str) -> dict[str, Any]:
    if CLOSURE.exists():
        raise HardStop("v8 live authorization already closed")
