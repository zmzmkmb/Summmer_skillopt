#!/usr/bin/env python3
"""Freeze TG8's zero-network observable-subgoal factorial design."""
...
CONFIG = ROOT / "configs/acl2027/tracegraph_tg8_observable_subgoal_preflight_v1.json"
OUTPUT_DIR = ROOT / "artifacts/acl2027_tracegraph_tg8_observable_subgoal_preflight_v1"
SCHEDULE_REL = "artifacts/acl2027_tracegraph_tg8_observable_subgoal_preflight_v1/development_schedule.json"
PREFLIGHT_REL = "artifacts/acl2027_tracegraph_tg8_observable_subgoal_preflight_v1/preflight.json"
MANIFEST_REL = "artifacts/acl2027_tracegraph_tg8_observable_subgoal_preflight_v1/completion_manifest.json"
FAMILIES = (
...
def validate_config(config: dict[str, Any], root: Path = ROOT) -> list[str]:
    errors: list[str] = []
    expected = {
        "phase_id": "TG8-tracegraph-observable-subgoal-preflight-v1",
        "experiment_line": "tracegraph-observable-state-skill-composition",
        "mode": "zero_network_design_only",
        "parent_completed_phase": "TG7-tracegraph-progress-memory-runner-v1",
        "task_family": "tg8_observable_subgoal_factorial",
        "family_split_quota": 4,
        "development_task_count": 24,
        "planned_episode_rows": 96,
    ...
    for split in ("valid_seen", "valid_unseen"):
        for family in FAMILIES:
            ...
            for row in candidates[:quota]:
                row["stratum"] = "tg8_multistep"
                selected.append(row)
    if len(selected) != 24 or len({row["task_identity"] for row in selected}) != 24:
        raise ValueError("TG8 schedule must contain 24 unique tasks")
    if any(row["task_identity"] in excluded for row in selected):
        raise ValueError("selected task overlaps an exclusion source")
    if sum(row["split"] == "valid_seen" for row in selected) != 12 or sum(row["split"] == "valid_unseen" for row in selected) != 12:
...
def run(config: dict[str, Any], root: Path = ROOT) -> dict[str, Any]:
    ...
    if any(path.exists() for path in paths):
        raise FileExistsError("TG8 preflight artifacts already exist")
    ...
    schedule = {
        ...
        "selection_rule": "first four lexicographically sorted fresh paired identities per family and split after TG4/TG6/TG7 exclusion",
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(f"VALID: froze {result['development_task_count']} fresh TG8 tasks and {result['planned_episode_rows']} rows; zero-network design only; execution_authorized=false")
    return 0
