#!/usr/bin/env python3
"""Authorization-gated live runner for the immutable v26 follow-up schedule."""
...
CALLS = 1600
STAGE_CEILING = 10.0
CUMULATIVE_CEILING = 10.0
INTERVAL_NS = 1_000_000_000
AUTH_ID = "phase2-post-v25-failure-analysis-followup-v27"
CONFIG = ROOT / "configs/acl2027/phase2_post_v25_failure_analysis_design_preflight_v26.json"
RECEIPT = ROOT / "configs/acl2027/phase2_post_v25_failure_analysis_user_authorization_receipt_v27.json"
...
MANIFEST = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_design_preflight_v26/run_manifest.json"
SCHEDULE = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_design_preflight_v26/future_schedule.json"
GOLD = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_design_preflight_v26/future_private_gold.json"
IDENTITY_AUDIT = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_design_preflight_v26/identity_overlap_audit.json"
ADAPTER = ROOT / "scripts/acl2027_phase2_token_plan_provider_adapter_v8.py"
ANALYZER = ROOT / "scripts/analyze_acl2027_phase2_post_v25_failure_analysis_v27.py"
TEST = ROOT / "tests/test_acl2027_phase2_post_v25_failure_analysis_live_v27.py"
...
def exact_contract() -> dict[str, Any]:
    return {"scope": "post_v25_failure_analysis_followup_only", "authorized_stage": "post_v25_failure_analysis_followup", "authorized_calls": CALLS, "max_provider_attempts": CALLS, "model_id": "qwen3.7-plus", "temperature": 0, "retries": 0, "max_tokens_present": False, "enable_thinking": False, "response_format": {"type": "json_object"}, "stage_cost_ceiling_cny": STAGE_CEILING, "cumulative_cost_ceiling_cny": CUMULATIVE_CEILING, "later_stages_authorized": False, "formal_scaling_authorized": False}


def preflight() -> dict[str, Any]:
    cfg, manifest, rows, identity, receipt = load(CONFIG), load(MANIFEST), load(SCHEDULE)["schedule"], load(IDENTITY_AUDIT), load(RECEIPT)
    expected_fingerprint = "6c27bceb77541e05cbb4b255796d806e72d83b95205744f10a7f877f49058564"
    if manifest.get("aggregate_fingerprint") != expected_fingerprint:
        raise HardStop("v26 fingerprint drift")
    if manifest.get("status") != "design-preflight-passed-closed" or manifest.get("future_logical_calls") != CALLS:
        raise HardStop("v26 manifest boundary drift")
    if cfg.get("execution", {}).get("provider_calls_allowed") is not False:
        raise HardStop("v26 must remain closed")
    if any(receipt.get(key) != value for key, value in exact_contract().items()) or receipt.get("status") != "explicit_user_authorization_received_execution_not_open":
        raise HardStop("v27 receipt boundary drift")
    if receipt.get("v26_aggregate_fingerprint") != expected_fingerprint:
        raise HardStop("v27 receipt fingerprint binding drift")
    expected_bindings = {
        "v26_config_sha256": sha256_file(CONFIG),
        "v26_manifest_sha256": sha256_file(MANIFEST),
        "v26_schedule_sha256": sha256_file(SCHEDULE),
        "v26_gold_sha256": sha256_file(GOLD),
        "v26_identity_audit_sha256": sha256_file(IDENTITY_AUDIT),
    }
    if receipt.get("bindings") != expected_bindings:
        raise HardStop("v27 receipt file binding drift")
    if any(receipt.get("execution", {}).get(key) is not False for key in ("network_calls_allowed", "provider_calls_allowed", "paid_api_allowed", "qwen_authorization_open", "formal_scaling_allowed")):
        raise HardStop("v27 receipt is not execution-closed")
    if any(identity.get(key) != 0 for key in ("prior_evaluation_task_overlap", "logical_call_id_overlap", "request_hash_overlap", "provider_response_id_overlap")):
        raise HardStop("v26 identity overlap drift")
    if len(rows) != CALLS or len({row["logical_call_id"] for row in rows}) != CALLS or len({row["request_hash"] for row in rows}) != CALLS:
        raise HardStop("v26 schedule identity drift")
    for index, row in enumerate(rows, 1):
        body = row["canonical_request_body"]
        if row.get("staged_execution_index") != index or row.get("partition") != "independent_failure_analysis_held_out" or row["request_hash"] != stable(body) or "max_tokens" in body or body.get("model_id") != "qwen3.7-plus" or body.get("temperature") != 0 or body.get("response_format") != {"type": "json_object"} or body.get("enable_thinking") is not False:
            raise HardStop("v26 request route/prefix drift")
    bindings = {"receipt_sha256": sha256_file(RECEIPT), "v26_config_sha256": sha256_file(CONFIG), "v26_manifest_sha256": sha256_file(MANIFEST), "v26_schedule_sha256": sha256_file(SCHEDULE), "v26_gold_sha256": sha256_file(GOLD), "v26_identity_audit_sha256": sha256_file(IDENTITY_AUDIT), "provider_adapter_sha256": sha256_file(ADAPTER), "analyzer_sha256": sha256_file(ANALYZER), "test_sha256": sha256_file(TEST), "live_runner_sha256": sha256_file(Path(__file__).resolve())}
    return {"schema_version": VERSION, "status": "zero-network-preflight-passed", "v26_aggregate_fingerprint": manifest["aggregate_fingerprint"], "authorized_calls": CALLS, "max_provider_attempts": CALLS, "stage_cost_ceiling_cny": STAGE_CEILING, "cumulative_cost_ceiling_cny": CUMULATIVE_CEILING, "bindings": bindings, "network_calls": 0, "provider_calls": 0, "model_calls": 0, "paid_api_calls": 0, "authorization_opened": False, "later_stages_authorized": False, "formal_scaling_authorized": False}


def authorization_template(key: str) -> dict[str, Any]:
    gate = preflight()
    return {"schema_version": VERSION, "experiment": "acl2027_phase2_post_v25_failure_analysis_live_v27", "authorization_id": AUTH_ID, "status": "open", "bindings": {**gate["bindings"], "zero_network_preflight_aggregate_fingerprint": stable(gate)}, "credential_sha256": hashlib.sha256(key.encode()).hexdigest(), **exact_contract(), "endpoint": "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1", "request_interval_seconds": 1.0, "paid_api_allowed": True, "provider_calls_allowed": True, "qwen_authorization_open": True, "formal_scaling_allowed": False, "forbidden_stages": ["calibration", "development_acquisition", "formal_history", "probe", "held_out", "other_models", "later_phase2_stages", "formal_scaling"]}
...
def open_authorization() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (AUTH, AUTH_CLOSED, REGISTRY, LEDGER, PACING, CLOSURE)):
        raise HardStop("v27 authorization already opened, started, or closed")
    gate = preflight()
...
def validate_authorization() -> tuple[dict[str, Any], str]:
    ...
    if not key or not AUTH.is_file() or not REGISTRY.is_file() or AUTH_CLOSED.exists() or CLOSURE.exists():
        raise HardStop("v27 authorization is closed or absent")
    auth = load(AUTH)
    if auth != authorization_template(key):
        raise HardStop("v27 authorization drift")
    item = load(REGISTRY)["authorizations"].get(AUTH_ID)
    if not item or item.get("sha256") != sha256_file(AUTH):
        raise HardStop("v27 authorization registry drift")
...
def execute(provider: Callable[[dict[str, Any]], dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    while len(records) < CALLS:
        if known_cost(records) >= min(STAGE_CEILING, CUMULATIVE_CEILING):
        ...
        try:
            ...
            record = {**base, "request_id": response.get("request_id") if isinstance(response, dict) else None, "raw_response": response.get("content") if isinstance(response, dict) else None, "raw_response_sha256": stable(response.get("content") if isinstance(response, dict) else None), "raw_provider_response": response, "usage": maybe, "local_cost_cny": cost(maybe) if maybe else None, "cumulative_local_cost_cny": round(known_cost(records) + (cost(maybe) if maybe else 0), 6) if maybe else None, "terminal": True, "status": "hard_stop", "error": f"{type(exc).__name__}: {exc}"}
        record["ledger_entry_sha256"] = stable(record)
        records.append(record)
        write(LEDGER, records)
        if record["terminal"]:
            raise HardStop(record["error"])
        if known_cost(records) >= min(STAGE_CEILING, CUMULATIVE_CEILING) and len(records) < CALLS:
...
def main() -> int:
    ...
    if args.command == "preflight":
        ...
        try:
            result = {"rows": len(execute(QwenTokenPlanProviderAdapter(load(AUTH)))), "closure": close("completed_exact_1600")}
        except Exception as exc:
