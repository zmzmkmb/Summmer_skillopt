#!/usr/bin/env python3
"""Run the authorized ACL 2027 Phase 1N two-call Plus confirmation."""
...
DEFAULT_CONFIG = ROOT / "configs/acl2027/phase1n_plus_live_confirmation_v1.json"
DEFAULT_STATE = ROOT / "paper/acl2027/experiment_state.json"
DEFAULT_OUTPUT = ROOT / "artifacts/acl2027_phase1n_plus_live_confirmation_v1"


class SmokeError(RuntimeError):
    """Raised when the bounded Phase 1N confirmation cannot proceed safely."""
...
def validate_inputs(config_path: Path = DEFAULT_CONFIG) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if config.get("phase") != "1N":
        raise SmokeError("config phase must be 1N")
    auth = config.get("authorization", {})
    execution = config.get("execution", {})
    if auth.get("authorized") is not True or auth.get("authorized_logical_calls") != 2:
        raise SmokeError("Phase 1N authorization must cover exactly two logical calls")
    if auth.get("max_provider_attempts") != 2 or auth.get("explicit_retries") != 0:
        raise SmokeError("Phase 1N provider attempt limit changed")
    if auth.get("qwen3.7_plus_authorized") is not True:
        raise SmokeError("Phase 1N Plus authorization is missing")
    closed = ("qwen3.8_max_authorized", "officeqa_24_authorized", "spreadsheetbench_40_authorized", "formal_scaling_authorized")
    if any(auth.get(key) is not False for key in closed):
        raise SmokeError("Phase 1N Max, batch, and scaling authorization must remain closed")
    if execution.get("paid_api_allowed") is not True or execution.get("network_calls_allowed") is not True:
        raise SmokeError("Phase 1N live confirmation permission is missing")
    if execution.get("formal_scaling_allowed") is not False or execution.get("sdk_max_retries") != 0:
        raise SmokeError("Phase 1N scaling or retry policy changed")

    if execution.get("omit_max_tokens") is not True:
        raise SmokeError("Phase 1N must omit max_tokens")
    ...
    if indices != [1, 2] or len(full_plan) != 2:
        raise SmokeError("Phase 1M request selection changed")
    ...
    for row in plan:
        ...
        if row.get("model") != frozen["model_id"] or row.get("request", {}).get("model") != frozen["model_id"]:
            raise SmokeError(f"{family} model is not frozen qwen3.7-plus")
        ...
        if canonical_hash(row["request"]) != row["request_hash"]:
            raise SmokeError(f"{family} request content changed after Phase 1M")
        if "max_tokens" in row.get("request", {}) or row.get("max_tokens_omitted") is not True:
            raise SmokeError(f"{family} request contains a forbidden max_tokens field")
    return config, plan
...
def assert_live_allowed(config: dict[str, Any], state_path: Path = DEFAULT_STATE) -> None:
    ...
    if phase.get("id") != "1N" or phase.get("status") != "in_progress":
        raise PermissionError("repository state is not Phase 1N in_progress")
    reason = str(policy.get("reason", "")).lower()
    if "exactly two" not in reason or "qwen3.7-plus" not in reason or "no retries" not in reason or "max_tokens" not in reason or "24" not in reason or "40" not in reason:
        raise PermissionError("repository authorization is not narrowly scoped to the two-call Plus confirmation")
...
def execute_smoke(
    *,
    ...
    manifest = {
        ...
        "artifact_role": "authorized_two_call_plus_confirmation_without_client_output_cap_only",
        "status": "in_progress",
        "started_at": utc_now(),
    ...
    for record in scientific_records:
        ...
        run_entries.append({
            "run_id": f"phase1n_plus_{int(record['call_index']):02d}_{str(record['task_family']).lower()}",
    ...
    manifest.update({
        ...
        "usage": usage,
        "decision": "plus_task_valid_floor_passed" if passed else "plus_task_valid_floor_failed_route_review_required",
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if not args.live_smoke or result["decision"] == "plus_task_valid_floor_passed" else 1
