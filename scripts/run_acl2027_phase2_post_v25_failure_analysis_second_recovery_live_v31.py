#!/usr/bin/env python3
"""Authorization-gated live runner for the immutable v30 second recovery."""
...
VERSION, CALLS = 31, 220
STAGE_CEILING, CUMULATIVE_CEILING, PRIOR_KNOWN_COST = 1.5, 8.0, 6.13433
INTERVAL_NS = 1_000_000_000
AUTH_ID = "phase2-post-v25-failure-analysis-second-recovery-v31"
RECEIPT = ROOT / "configs/acl2027/phase2_post_v25_failure_analysis_second_recovery_user_authorization_receipt_v31.json"
V30_CONFIG = ROOT / "configs/acl2027/phase2_post_v25_failure_analysis_recovery_preflight_v30.json"
V30_ARTIFACT = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_recovery_preflight_v30"
MANIFEST = V30_ARTIFACT / "run_manifest.json"
SCHEDULE = V30_ARTIFACT / "second_recovery_schedule.json"
GOLD = V30_ARTIFACT / "combined_private_gold.json"
V27_LEDGER = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_live_v27/ledger.json"
V29_LEDGER = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_recovery_live_v29/ledger.json"
V29_CLOSURE = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_recovery_live_v29/authorization_closure.json"
V29_AUDIT = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_recovery_live_v29/run_audit.json"
ADAPTER = ROOT / "scripts/acl2027_phase2_token_plan_provider_adapter_v8.py"
ANALYZER = ROOT / "scripts/analyze_acl2027_phase2_post_v25_failure_analysis_second_recovery_v31.py"
TEST = ROOT / "tests/test_acl2027_phase2_post_v25_failure_analysis_second_recovery_live_v31.py"
ARTIFACT = ROOT / "artifacts/acl2027_phase2_post_v25_failure_analysis_second_recovery_live_v31"
...
def exact_contract() -> dict[str, Any]:
    return {"scope": "post_v25_failure_analysis_second_recovery_only", "authorized_stage": "post_v25_failure_analysis_second_recovery", "authorized_calls": CALLS, "max_provider_attempts": CALLS, "model_id": "qwen3.7-plus", "temperature": 0, "retries": 0, "max_tokens_present": False, "enable_thinking": False, "response_format": {"type": "json_object"}, "stage_cost_ceiling_cny": STAGE_CEILING, "cumulative_cost_ceiling_cny": CUMULATIVE_CEILING, "known_cumulative_cost_lower_bound_cny": PRIOR_KNOWN_COST, "later_stages_authorized": False, "formal_scaling_authorized": False}


def preflight() -> dict[str, Any]:
    cfg, manifest, rows, receipt = load(V30_CONFIG), load(MANIFEST), load(SCHEDULE)["schedule"], load(RECEIPT)
    fingerprint = "d008dd5fc9cdd765a1dcc3c09da207e983c550a4fe77558da4cfb78f66bae23a"
    if manifest.get("aggregate_fingerprint") != fingerprint or manifest.get("status") != "preflight-passed-recovery-plan-closed":
        raise HardStop("v30 manifest boundary drift")
    if any(value is not False for value in cfg.get("execution", {}).values()):
        raise HardStop("v30 must remain execution-closed")
    if receipt.get("status") != "explicit_user_authorization_received_execution_not_open" or receipt.get("v30_aggregate_fingerprint") != fingerprint or any(receipt.get(key) != value for key, value in exact_contract().items()):
        raise HardStop("v31 receipt boundary drift")
    expected = {"v30_manifest_sha256": sha256_file(MANIFEST), "v30_schedule_sha256": sha256_file(SCHEDULE), "v30_gold_sha256": sha256_file(GOLD)}
    if receipt.get("bindings") != expected:
        raise HardStop("v31 receipt binding drift")
    ...
    if {row["logical_call_id"] for row in rows} & {row["logical_call_id"] for row in spent} or {row["request_hash"] for row in rows} & {row["request_hash"] for row in spent}:
        raise HardStop("v31 retries a spent request")
    if load(V29_CLOSURE).get("status") != "closed" or load(V29_AUDIT).get("status") != "terminal_hard_stop":
        raise HardStop("v29 terminal provenance drift")
    for index, row in enumerate(rows, 1):
        body = row["canonical_request_body"]
        if row.get("staged_execution_index") != index or row.get("partition") != "independent_failure_analysis_held_out_second_recovery" or row["request_hash"] != stable(body) or "max_tokens" in body or body.get("model_id") != "qwen3.7-plus" or body.get("temperature") != 0 or body.get("response_format") != {"type": "json_object"} or body.get("enable_thinking") is not False:
            raise HardStop("v30 route/prefix drift")
    bindings = {"receipt_sha256": sha256_file(RECEIPT), "v30_config_sha256": sha256_file(V30_CONFIG), "v30_manifest_sha256": sha256_file(MANIFEST), "v30_schedule_sha256": sha256_file(SCHEDULE), "v30_gold_sha256": sha256_file(GOLD), "v27_ledger_sha256": sha256_file(V27_LEDGER), "v29_ledger_sha256": sha256_file(V29_LEDGER), "v29_closure_sha256": sha256_file(V29_CLOSURE), "v29_run_audit_sha256": sha256_file(V29_AUDIT), "provider_adapter_sha256": sha256_file(ADAPTER), "analyzer_sha256": sha256_file(ANALYZER), "test_sha256": sha256_file(TEST), "live_runner_sha256": sha256_file(Path(__file__).resolve())}
    return {"schema_version": VERSION, "status": "zero-network-preflight-passed", "v30_aggregate_fingerprint": fingerprint, "authorized_calls": CALLS, "max_provider_attempts": CALLS, "stage_cost_ceiling_cny": STAGE_CEILING, "cumulative_cost_ceiling_cny": CUMULATIVE_CEILING, "known_cumulative_cost_lower_bound_cny": PRIOR_KNOWN_COST, "bindings": bindings, "network_calls": 0, "provider_calls": 0, "model_calls": 0, "paid_api_calls": 0, "authorization_opened": False, "later_stages_authorized": False, "formal_scaling_authorized": False}


def authorization_template(key: str) -> dict[str, Any]:
    gate = preflight()
    return {"schema_version": VERSION, "experiment": "acl2027_phase2_post_v25_failure_analysis_second_recovery_live_v31", "authorization_id": AUTH_ID, "status": "open", "bindings": {**gate["bindings"], "zero_network_preflight_aggregate_fingerprint": stable(gate)}, "credential_sha256": hashlib.sha256(key.encode()).hexdigest(), **exact_contract(), "endpoint": "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1", "request_interval_seconds": 1.0, "paid_api_allowed": True, "provider_calls_allowed": True, "qwen_authorization_open": True, "formal_scaling_allowed": False, "forbidden_stages": ["calibration", "development_acquisition", "formal_history", "probe", "held_out", "other_models", "later_phase2_stages", "formal_scaling"]}
...
def open_authorization() -> dict[str, Any]:
    ...
    if any(path.exists() for path in (AUTH, AUTH_CLOSED, REGISTRY, LEDGER, PACING, CLOSURE)):
        raise HardStop("v31 authorization already opened, started, or closed")
    gate = preflight()
...
def validate_authorization() -> tuple[dict[str, Any], str]:
    ...
    if not key or not AUTH.is_file() or not REGISTRY.is_file() or AUTH_CLOSED.exists() or CLOSURE.exists():
        raise HardStop("v31 authorization is closed or absent")
    auth = load(AUTH)
    if auth != authorization_template(key):
        raise HardStop("v31 authorization drift")
    item = load(REGISTRY)["authorizations"].get(AUTH_ID)
    if not item or item.get("sha256") != sha256_file(AUTH):
        raise HardStop("v31 authorization registry drift")
...
def main() -> int:
    ...
    if args.command == "preflight":
        ...
        try:
            result = {"rows": len(execute(QwenTokenPlanProviderAdapter(load(AUTH)))), "closure": close("completed_exact_220")}
        except Exception as exc:
