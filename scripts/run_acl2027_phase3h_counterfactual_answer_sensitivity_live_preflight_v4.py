#!/usr/bin/env python3
"""Freeze the zero-network Phase 3H live-execution preflight v4."""
...
CONFIG = ROOT / "configs/acl2027/phase3h_counterfactual_answer_sensitivity_live_preflight_v4.json"
SCRIPT = ROOT / "scripts/run_acl2027_phase3h_counterfactual_answer_sensitivity_live_preflight_v4.py"
TEST = ROOT / "tests/test_acl2027_phase3h_counterfactual_answer_sensitivity_live_preflight_v4.py"
SOURCE = ROOT / "artifacts/acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v3"
SOURCE_MANIFEST = SOURCE / "run_manifest.json"
SOURCE_SCHEDULE = SOURCE / "counterfactual_schedule.json"
SOURCE_GOLD = SOURCE / "counterfactual_private_gold.json"
...
LIVE_ARTIFACT = ROOT / "artifacts/acl2027_phase3h_counterfactual_answer_sensitivity_live_v5"
AUTH_OPEN = LIVE_ARTIFACT / "authorization_open.json"
ARTIFACT = ROOT / "artifacts/acl2027_phase3h_counterfactual_answer_sensitivity_live_preflight_v4"
REPORT = ROOT / "paper/acl2027/results/phase3h_counterfactual_answer_sensitivity_live_preflight_v4.md"
...
def schedule_rows() -> list[dict[str, Any]]:
    rows = load(SOURCE_SCHEDULE).get("schedule")
    if not isinstance(rows, list):
        raise RuntimeError("Phase 3H v3 schedule is not a list")
    return rows


def validate_schedule(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if len(rows) != CALLS:
        raise RuntimeError("Phase 3H v3 schedule row-count drift")
    sequences = [row.get("sequence") for row in rows]
    if sequences != list(range(1, CALLS + 1)):
        raise RuntimeError("Phase 3H v3 sequence drift")
    ...
    if len(set(logical_ids)) != CALLS or len(set(request_hashes)) != CALLS:
        raise RuntimeError("Phase 3H v3 request identities are not unique")
    for row in rows:
        ...
        contract = body.get("response_contract", {})
        if contract.get("skill_assessments", {}).get("type") != "object":
            raise RuntimeError(f"candidate mapping contract drift at sequence {row.get('sequence')}")
    ...
    if len(task_ids) != 40 or condition_counts != Counter({condition: 40 for condition in CONDITIONS}):
        raise RuntimeError("Phase 3H v3 task-grid balance drift")
    return {
        "rows": len(rows),
        "tasks": len(task_ids),
...
def validate() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if source_manifest.get("aggregate_fingerprint") != DESIGN_FINGERPRINT:
        raise RuntimeError("Phase 3H design fingerprint drift")
    if source_completion.get("aggregate_fingerprint") != DESIGN_FINGERPRINT:
        raise RuntimeError("Phase 3H completion fingerprint drift")
    if source_manifest.get("status") != "design-preflight-passed-closed":
        raise RuntimeError("Phase 3H design is not closed and complete")
    if source_completion.get("completed_calls") != CALLS or source_completion.get("rows") != CALLS:
        raise RuntimeError("Phase 3H aggregate design-row completion drift")
    if source_completion.get("provider_calls_executed") != 0:
        raise RuntimeError("Phase 3H source unexpectedly contains provider execution")
    if cfg.get("status") != "zero_network_live_execution_preflight_closed":
        raise RuntimeError("v4 preflight status drift")
    if any(value is not False for value in cfg["execution"].values()):
        raise RuntimeError("v4 execution switches must remain closed")
    authorization = cfg["authorization"]
    if authorization.get("receipt_created") is not False or authorization.get("authorization_open") is not False:
        raise RuntimeError("v4 authorization must remain closed")
    if any(authorization.get(key) is not False for key in (
        "data_egress_authorized", "provider_calls_authorized", "paid_api_authorized"
    )):
        raise RuntimeError("v4 authorization permissions must remain false")
    if RECEIPT.exists() or AUTH_OPEN.exists():
        raise RuntimeError("Phase 3H v5 authorization receipt or open authorization already exists")

    contract = cfg["execution_contract"]
    expected_contract = {
        "scope": "phase3h_counterfactual_answer_sensitivity_only",
        "endpoint": ENDPOINT,
        "model_id": MODEL,
    ...
    if any(contract.get(key) is not True for key in (
        ...
        raise RuntimeError("v4 safety or accounting contract drift")
    ...
    result = {
        ...
        "phase3h_design_fingerprint": DESIGN_FINGERPRINT,
...
def report_text(result: dict[str, Any], request: dict[str, Any]) -> str:
    return f"""# ACL 2027 Phase 3H live-execution preflight v4

This zero-network preflight binds all 200 immutable Phase 3H v3 requests without changing their canonical identities. It freezes the Token Plan Beijing endpoint, `qwen3.7-plus`, temperature 0, thinking disabled, zero retries, absent `max_tokens`, JSON-object responses, one-second pacing, CNY 3.00 stage ceiling, and CNY 15.00 cumulative ceiling.
