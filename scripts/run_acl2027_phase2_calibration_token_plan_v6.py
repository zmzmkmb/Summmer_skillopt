#!/usr/bin/env python3
"""Phase 2 calibration-only runner for the Beijing Token Plan route."""
...
from scripts.run_acl2027_phase2_token_plan_route_preflight_v6 import CONFIG, ARTIFACT, build_manifest, validate_config
...
def required_bindings() -> dict[str, str]:
    ...
    return {
        "route_preflight_config_sha256": sha256_file(CONFIG),
        "route_preflight_manifest_sha256": sha256_file(PREFLIGHT_MANIFEST),
        "parent_v4_aggregate_fingerprint": manifest["v4_aggregate_fingerprint"],
        "schedule_sha256": sha256_file(ROOT / "artifacts/acl2027_phase2_staged_live_runner_preflight_v2/staged_execution_schedule.json"),
...
def validate_execution_preflight() -> dict[str, Any]:
    ...
    if manifest != build_manifest():
        raise HardStop("v6 route preflight manifest drift")
    if manifest["status"] != "preflight-passed-closed" or any(manifest["counters"].values()):
        raise HardStop("v6 route preflight not closed")
    return config
    ...
    *,
    ...
    while len(records) < 60:
        ...
        if "max_tokens" in body or body.get("model_id") != "qwen3.7-plus" or body.get("temperature") != 0:
            raise HardStop("v6 request boundary drift", records)
        ...
        if cost(config, records) > auth["stage_cost_ceiling_cny"] or cost(config, records) > auth["cumulative_cost_ceiling_cny"]:
            ...
            records[-1]["error"] = "v6 cost ceiling exceeded"
