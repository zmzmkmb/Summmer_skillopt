#!/usr/bin/env python3
"""Authorization-gated live runner for the immutable Phase 4B development schedule."""
...
VERSION, CALLS = 2, 100
DESIGN_FINGERPRINT = "2a7a6814b261ad336902fa8c4c6048a6114f62ffa242aae267a3700d0e97a2db"
PREFLIGHT_FINGERPRINT = "0826f7a4cd5b673ce2e2dfed50a0cecd8806b0b8176c1ac004987b3a0f8bd243"
ENDPOINT = "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
STAGE_CEILING, CUMULATIVE_CEILING, PRIOR_COST = 2.0, 15.0, 11.339420
INTERVAL_NS = 1_000_000_000
AUTH_ID = "phase4b-development-live-v2"
PAYLOAD_CLASSES = ["frozen_task_prompts", "frozen_task_contexts", "frozen_skill_candidate_bundles"]
AUTHORIZATION_STATEMENT = (
    "User explicitly authorizes the 100 Phase 4B development calls bound to the full Phase 4A fingerprint "
    f"{DESIGN_FINGERPRINT}, the full Phase 4B preflight fingerprint {PREFLIGHT_FINGERPRINT}, and the frozen "
    "qwen3.7-plus Token Plan contract, including paid API/provider calls, temperature 0, disabled thinking, "
...
SCHEDULE = PREFLIGHT / "development_schedule.json"
...
RECEIPT = ROOT / "configs/acl2027/phase4b_development_user_authorization_receipt_v2.json"
ARTIFACT = ROOT / "artifacts/acl2027_phase4b_development_live_v2"
...
def contract() -> dict[str, Any]:
    return {
        "scope": "phase4b_development_calibration_only",
...
def preflight() -> list[dict[str, Any]]:
    ...
    if request.get("phase4a_design_fingerprint") != DESIGN_FINGERPRINT or request.get("phase4b_live_preflight_fingerprint") != PREFLIGHT_FINGERPRINT:
...
def receipt() -> dict[str, Any]:
    return {
        ...
        "phase4a_design_fingerprint": DESIGN_FINGERPRINT,
...
def authorize() -> dict[str, Any]:
    ...
    if ARTIFACT.exists() or RECEIPT.exists():
        raise HardStop("Phase 4B v2 authorization or execution record already exists")
...
def validate_open() -> tuple[list[dict[str, Any]], str]:
    ...
    if not key or not AUTH.exists() or not REGISTRY.exists() or CLOSURE.exists():
        raise HardStop("Phase 4B v2 authorization is not open")
    auth = load(AUTH)
    if auth.get("credential_sha256") != hashlib.sha256(key.encode()).hexdigest() or not all(auth.get(k) is True for k in ("paid_api_allowed", "provider_calls_allowed", "qwen_authorization_open")):
        raise HardStop("Phase 4B v2 open authorization drift")
