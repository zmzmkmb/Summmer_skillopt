#!/usr/bin/env python3
"""Run the ACL 2027 Phase 1B SearchQA Token Plan v5 pilot.

V5 retains the frozen v4 task protocol while adding explicit request pacing,
registered rate-limit backoff, terminal content-inspection rejection records,
and conservative full-envelope accounting whenever provider usage is unknown.
"""
...
DEFAULT_CONFIG = (
    PROJECT_ROOT / "configs" / "acl2027" / "searchqa_phase1b_token_plan_live_pilot_v5.json"
)
...
def validate_config(config_path: Path = DEFAULT_CONFIG, root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    transport = config.get("transport_policy", {})
    _require(Decimal(str(transport.get("min_request_interval_seconds"))) == Decimal("1.0"), "v5 request interval must be 1 second")
    _require(transport.get("sdk_max_retries") == 0, "SDK retries must remain disabled")
    _require(transport.get("retry_http_statuses") == [429], "only HTTP 429 is transport-retriable")
    _require(int(transport.get("max_rate_limit_retries_per_call", -1)) == 2, "v5 rate-limit retry cap must be two")
    _require(Decimal(str(transport.get("backoff_initial_seconds"))) == Decimal("2.0"), "unexpected initial 429 backoff")
    _require(Decimal(str(transport.get("backoff_multiplier"))) == Decimal("2.0"), "unexpected 429 backoff multiplier")
    rejection = config.get("provider_rejection_policy", {})
    _require(rejection.get("terminal_error_codes") == ["data_inspection_failed"], "unexpected terminal rejection codes")
    _require(rejection.get("retry_same_content") is False, "content-rejected input must not be retried")
    _require(rejection.get("keep_in_metric_denominator") is True, "provider rejection must remain in denominator")
    _require(rejection.get("metric_value") == 0.0, "provider rejection metrics must be zero")
    _require(rejection.get("unknown_usage_accounting") == "conservative_full_envelope", "unknown usage must use the full frozen envelope")
    ...
    _verify_file_hash(
        root,
        config["transport_development_fixture"],
        "path",
        "sha256",
    )
...
def validate_call_record(record: dict[str, Any]) -> None:
    ...
    terminal_rejection = record.get("outcome") == "provider_rejected"
    expected_successes = 0 if terminal_rejection else 1
    if successful != expected_successes or successful + failed != len(attempts):
        raise ResumeError(f"run {record.get('run_id')} has invalid attempt statuses")
    if terminal_rejection and attempts[-1].get("provider_error_code") != "data_inspection_failed":
        raise ResumeError(f"run {record.get('run_id')} lacks a terminal inspection rejection")
    ...
    expected_parse = "provider_rejected" if terminal_rejection else "valid"
    if record.get("parse_status") != expected_parse:
        raise ResumeError(f"completed run {record.get('run_id')} has invalid parse status")
...
def ledger_from_attempts(
    ...
    ledger["provider_rejected_calls"] = sum(
        record.get("outcome") == "provider_rejected" for record in completed_rows
    )
...
def _assert_ledger_identities(ledger: dict[str, int]) -> None:
    ...
    if ledger["logical_calls"] > (
        ledger["successful_attempts"] + ledger.get("provider_rejected_calls", 0)
    ):
        raise PreflightError("logical calls exceed successful attempts plus terminal rejections")
...
class LiveAttempt:
    ...
    provider_error_id: str = ""
    http_status: int | None = None


def _provider_error_details(exc: Exception) -> tuple[int | None, str, str]:
    ...
    if isinstance(body, dict):
        ...
        if isinstance(error_body, dict):
            code = str(error_body.get("code", code) or code)
    rendered = str(exc)
    if not code:
        match = re.search(r"(?:code[=: ]+|['\"]code['\"]\s*:\s*['\"])([A-Za-z0-9_.-]+)", rendered)
        code = match.group(1) if match else ""
    request_id = str(getattr(exc, "request_id", "") or "")
    if not request_id:
        match = re.search(r"chatcmpl-[A-Za-z0-9-]+", rendered)
        request_id = match.group(0) if match else ""
    return status, code, request_id


class DashScopePaygProvider:
    """One-request adapter with explicit start-time pacing and no SDK retries."""
    ...
    def _usage(response: Any) -> tuple[int, int]:
        ...
        if total_tokens != input_tokens + output_tokens:
            raise ProviderUsageError("provider usage identity failed: total != prompt + completion")
    ...
    def invoke(
        ...
        *,
        ...
        try:
            ...
            status, code, request_id = _provider_error_details(exc)
            return LiveAttempt(
                status="failed",
                response="",
                input_tokens=0,
...
def _is_rate_limit_attempt(attempt: dict[str, Any]) -> bool:
    return int(attempt.get("http_status") or 0) == 429


def _is_terminal_rejection(attempt: dict[str, Any]) -> bool:
    return attempt.get("provider_error_code") == "data_inspection_failed"
    ...
    *,
    ...
    record = {
        **call,
        ...
        "provider": "dashscope_beijing_token_plan" if mode == "live" else "deterministic_no_network",
        "paid_call": mode == "live",
        "attempts": attempts,
        "provider_attempts": len(attempts),
        "failed_attempts": sum(attempt["status"] == "failed" for attempt in attempts),
        "retry_count": len(attempts) - 1,
        "parse_status": "provider_rejected",
        "predicted_answer": "",
        "gold_answers": item["answers"],
        "em": 0.0,
        "f1": 0.0,
        "sub_em": 0.0,
        "provider_error_code": terminal.get("provider_error_code", ""),
        "provider_error_id": terminal.get("provider_error_id", ""),
        "prompt_fingerprint": prepared["prompt_fingerprint"],
        "system_prompt_sha256": prepared["system_prompt_sha256"],
        "user_prompt_sha256": prepared["user_prompt_sha256"],
        "active_skill_sha256": prepared["active_skill_sha256"],
    ...
    for field in (
        ...
        "input_cost_pico_cny", "output_cost_pico_cny", "cost_pico_cny",
        "input_cost_pico_usd", "output_cost_pico_usd", "cost_pico_usd",
    ):
        record[field] = sum(int(attempt[field]) for attempt in attempts)
    validate_call_record(record)
    return record
...
def execute_prepared_call(
    ...
    *,
    ...
    if attempts and _is_terminal_rejection(attempts[-1]):
        return _provider_rejection_record(call, item, prepared, attempts, mode=mode)
    max_attempts = int(config["model"]["max_attempts_per_logical_call"])
    ...
    for attempt_index in range(len(attempts) + 1, max_attempts + 1):
        ...
        if prior and _is_rate_limit_attempt(prior):
            rate_limit_count = sum(_is_rate_limit_attempt(attempt) for attempt in attempts)
            policy = config["transport_policy"]
            if rate_limit_count > int(policy["max_rate_limit_retries_per_call"]):
                raise PreflightError(f"rate-limit retry cap exhausted: {call['run_id']}")
            delay = float(policy["backoff_initial_seconds"]) * (
                float(policy["backoff_multiplier"]) ** (rate_limit_count - 1)
            )
            provider.sleep(delay) if hasattr(provider, "sleep") else time.sleep(delay)
        use_repair = bool(
            prior
        ...
        accounted_result = result
        if not bool(getattr(result, "usage_known", True)):
            accounted_result = LiveAttempt(
                status=result.status,
                response=result.response,
                input_tokens=int(reservation["input_tokens"]),
                output_tokens=int(reservation["output_tokens"]),
                error=result.error,
                usage_known=False,
                reasoning_content=str(getattr(result, "reasoning_content", "") or ""),
                request_id=str(getattr(result, "request_id", "") or ""),
                provider_error_code=str(getattr(result, "provider_error_code", "") or ""),
                provider_error_id=str(getattr(result, "provider_error_id", "") or ""),
        ...
        attempt.update(
            {
                ...
                "provider_error_id": str(getattr(result, "provider_error_id", "") or ""),
                "http_status": getattr(result, "http_status", None),
                "provider_input_tokens": int(result.input_tokens) if bool(getattr(result, "usage_known", True)) else None,
                "provider_output_tokens": int(result.output_tokens) if bool(getattr(result, "usage_known", True)) else None,
                "token_accounting": "exact_provider_usage" if bool(getattr(result, "usage_known", True)) else "conservative_full_envelope",
            }
        )
        ...
        if _is_terminal_rejection(attempt):
            return _provider_rejection_record(call, item, prepared, attempts, mode=mode)
        if not attempt["usage_known"] and not _is_rate_limit_attempt(attempt):
            raise ProviderUsageError(
                "unclassified provider failure omitted usage metadata; the full envelope "
                "is journaled, but further paid execution is refused"
            )
...
def _provider_for_mode(config: dict[str, Any], mode: str) -> Any:
    ...
    return DashScopePaygProvider(
        ...
        min_request_interval_seconds=float(
            config["transport_policy"]["min_request_interval_seconds"]
        ),
    )
...
def execute_preflight(
    *,
    ...
    malformed_unknown_usage = [
        identity
        for identity in unknown_usage
        if attempts_by_id[identity[0]][identity[1] - 1].get("token_accounting")
        != "conservative_full_envelope"
    ]
    if malformed_unknown_usage:
        raise ProviderUsageError(
            "unknown-usage attempts lack conservative full-envelope accounting: "
            f"{malformed_unknown_usage[:3]}"
        )
    ...
    with calls_path.open("a", encoding="utf-8", newline="\n") as handle:
        for call in plan:
            ...
            ledger["provider_rejected_calls"] += int(
                record.get("outcome") == "provider_rejected"
            )
            _assert_ledger_identities(ledger)
            new_calls += 1
    ...
    unknown_usage = [
        ...
        for run_id, attempts in attempts_by_id.items()
        for attempt in attempts
        if not attempt.get("usage_known", True)
    ]
    summary = {
        "artifact_schema_version": 2,
        "experiment": config["experiment"],
        ...
        "transport_accounting": {
            "unknown_usage_attempts": sum(
                not attempt.get("usage_known", True)
                for attempts in attempts_by_id.values()
                for attempt in attempts
            ),
            "conservatively_accounted_attempts": sum(
                attempt.get("token_accounting") == "conservative_full_envelope"
                for attempts in attempts_by_id.values()
                for attempt in attempts
            ),
            "rate_limit_attempts": sum(
                _is_rate_limit_attempt(attempt)
                for attempts in attempts_by_id.values()
                for attempt in attempts
            ),
            "provider_rejected_calls": sum(
                record.get("outcome") == "provider_rejected"
                for record in completed.values()
            ),
        },
        "gates": {
            ...
            "exact_accounting": not bool(unknown_usage),
            "conservative_unknown_usage_accounting": all(
                attempt.get("token_accounting") == "conservative_full_envelope"
                for attempts in attempts_by_id.values()
                for attempt in attempts
                if not attempt.get("usage_known", True)
            ),
            "strict_parser_or_registered_rejection": True,
