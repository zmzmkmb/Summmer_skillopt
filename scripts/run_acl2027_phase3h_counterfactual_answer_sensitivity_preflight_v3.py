#!/usr/bin/env python3
"""Completion-schema-corrected Phase 3H zero-network preflight."""
...
VERSION = 3
EXPERIMENT = "acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v3"
CONFIG = ROOT / "configs/acl2027/phase3h_counterfactual_answer_sensitivity_preflight_v3.json"
SCRIPT = ROOT / "scripts/run_acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v3.py"
TEST = ROOT / "tests/test_acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v3.py"
ARTIFACT = ROOT / "artifacts/acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v3"
REPORT = ROOT / "paper/acl2027/results/phase3h_counterfactual_answer_sensitivity_preflight_v3.md"
...
V2_MANIFEST = ROOT / "artifacts/acl2027_phase3h_counterfactual_answer_sensitivity_preflight_v2/run_manifest.json"
...
def selector_hash(family: str, task_id: str) -> str:
    return hashlib.sha256(f"phase3h-v3:counterfactual-answer:{family}:{task_id}".encode()).hexdigest()
...
def candidate_payloads(family: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for candidate in (target, control):
        candidate["candidate_id"] = str(candidate["candidate_id"]).replace("phase3h-target", "phase3h-v3-target").replace("phase3h-inverse-control", "phase3h-v3-inverse-control")
...
def build_schedule(tasks: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for row in schedule:
        ...
        body["candidate_version"] = "phase3h-v3-design-only"
        body["prompt_template_version"] = "phase3h-counterfactual-grounding-native-mapping-v3"
        row["logical_call_id"] = str(row["logical_call_id"]).replace("phase3h-v1:", "phase3h-v3:")
...
def source_paths() -> dict[str, Path]:
    return {
        ...
        "v2_unaccepted_manifest_sha256": V2_MANIFEST,
...
def write_artifact(result: dict[str, Any]) -> None:
    ...
    documents = {
        ...
        "completion_manifest.json": {
            ...
            "proposed_calls": len(schedule),
            "completed_calls": len(schedule),
            "rows": len(schedule),
    ...
    for name, value in documents.items():
        (ARTIFACT / name).write_text(base.render(value), encoding="utf-8", newline="\n")
    REPORT.write_text(base.report_text(result), encoding="utf-8", newline="\n")
...
def _configure_base() -> None:
    ...
    base.write_artifact = write_artifact
