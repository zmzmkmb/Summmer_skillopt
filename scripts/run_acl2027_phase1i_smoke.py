#!/usr/bin/env python3
"""Run the authorized ACL 2027 Phase 1I repaired two-call smoke."""
...
DEFAULT_CONFIG = ROOT / "configs/acl2027/phase1i_repaired_cross_task_live_smoke_v1.json"
DEFAULT_STATE = ROOT / "paper/acl2027/experiment_state.json"
DEFAULT_OUTPUT = ROOT / "artifacts/acl2027_phase1i_repaired_cross_task_live_smoke_v1"


class SmokeError(RuntimeError):
    """Raised when the bounded Phase 1I smoke cannot proceed safely."""
...
def validate_inputs(config_path: Path = DEFAULT_CONFIG) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if config.get("phase") != "1I":
        raise SmokeError("config phase must be 1I")
    auth = config.get("authorization", {})
    execution = config.get("execution", {})
    if auth.get("authorized") is not True or auth.get("authorized_logical_calls") != 2:
        raise SmokeError("Phase 1I authorization must cover exactly two logical calls")
    if auth.get("max_provider_attempts") != 2 or auth.get("explicit_retries") != 0:
        raise SmokeError("Phase 1I provider attempt limit changed")
    if any(auth.get(key) is not False for key in ("officeqa_24_authorized", "spreadsheetbench_40_authorized", "formal_scaling_authorized")):
        raise SmokeError("Phase 1I batch or scaling authorization must remain closed")
    if execution.get("paid_api_allowed") is not True or execution.get("network_calls_allowed") is not True:
        raise SmokeError("Phase 1I live smoke permission is missing")
    if execution.get("formal_scaling_allowed") is not False or execution.get("sdk_max_retries") != 0:
        raise SmokeError("Phase 1I scaling or retry policy changed")

    frozen = config["frozen_preflight"]
    for path_key, hash_key in (("config_path", "config_sha256"), ("payload_review_path", "payload_review_sha256")):
        if sha256_file(ROOT / frozen[path_key]) != frozen[hash_key]:
            raise SmokeError(f"frozen preflight {path_key} hash mismatch")
    review = read_json(ROOT / frozen["payload_review_path"])
    plan = review.get("requests", [])
    ...
    for row in plan:
        ...
        if canonical_hash(row["request"]) != row["request_hash"]:
            raise SmokeError(f"{family} request content changed after Phase 1H")
    return config, plan
...
def assert_live_allowed(config: dict[str, Any], state_path: Path = DEFAULT_STATE) -> None:
    ...
    if phase.get("id") != "1I" or phase.get("status") != "in_progress":
        raise PermissionError("repository state is not Phase 1I in_progress")
    reason = str(policy.get("reason", "")).lower()
    if "exactly two" not in reason or "24" not in reason or "40" not in reason:
        raise PermissionError("repository authorization is not narrowly scoped to the repaired two-call smoke")
...
def execute_smoke(
    *,
    ...
    manifest = {
        ...
        "artifact_role": "authorized_repaired_two_call_smoke_only",
        "status": "in_progress",
        "started_at": utc_now(),
    ...
    manifest.update({
        ...
        "usage": usage,
        "decision": "repaired_smoke_passed_batch_still_closed" if passed else "repaired_smoke_failed_batch_closed",
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if not args.live_smoke or result["decision"] == "repaired_smoke_passed_batch_still_closed" else 1
