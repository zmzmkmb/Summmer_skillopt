...
PUT_ALIAS_RE = re.compile(r"^(?:move|put|place)\s+(.+?)\s+(?:to|in|into|on|onto)\s+(.+)$")
...
LEDGER_FINGERPRINT_FIELD = "ledger_integrity_fingerprint"
...
def canonical_command(command: str) -> str:
    ...
    match = PUT_ALIAS_RE.match(text)
    if match and text.startswith("move "):
        return f"put {match.group(1)} in {match.group(2)}"
    return text


def command_family(command: str) -> str:
    text = canonical_command(command)
    return base.command_family(text)


def matching_skill_ids(index: base.SkillIndex, command: str) -> list[str]:
    matches = index.matching_skill_ids(command)
    if matches:
        return matches
    canonical = canonical_command(command)
    if canonical != normalize(command):
        return index.matching_skill_ids(canonical)
    return []
...
def extract_task_entities(observation: str) -> tuple[str, list[str], list[str], list[str]]:
    ...
    verb_positions = [
        ...
        if word in {"put", "place", "move", "carry", "wash", "washed", "rinse", "rinsed", "clean", "cleaned", "retrieve", "find"}
    ]
    first_verb = verb_positions[0] if verb_positions else 0
    next_connector = next(
        (index for index in range(first_verb + 1, len(words)) if words[index] in {"and", "from", "in", "into", "on", "onto", "to"}),
        len(words),
    )
    ignored = {"a", "an", "the", "some", "two", "both"} | clean_words
    object_words = [word for word in words[first_verb + 1:next_connector] if word not in ignored]
    # Resolve pronouns in multi-step natural-language task descriptions.
    if object_words in (["it"], ["them"]):
        prior = [word for word in words[:first_verb] if word not in ignored and word not in {"and", "then"}]
        object_words = prior[-2:] or object_words
    object_tokens = sorted({canonical_token(word) for word in object_words if word not in {"it", "them"}})
    destination_start = next(
        (index + 1 for index in range(next_connector, len(words)) if words[index] in {"in", "into", "on", "onto", "to"}),
        len(words),
    )
    destination_end = next(
        (index for index in range(destination_start, len(words)) if words[index] in {"and", "then", ","}),
        len(words),
    )
    destination_tokens = sorted({canonical_token(word) for word in words[destination_start:destination_end] if word not in ignored and word not in {"it", "them"}})
...
def command_matches(command: str, family: str, entities: list[str]) -> bool:
    return command_family(command) == family and bool(token_overlap(command, entities))
...
def ledger_fingerprint(ledger: dict[str, Any]) -> str:
    """Return a self-hash over the ledger excluding its stored hash field."""
    payload = dict(ledger)
    payload.pop(LEDGER_FINGERPRINT_FIELD, None)
    return base.fingerprint(payload)


def seal_ledger(ledger: dict[str, Any]) -> dict[str, Any]:
    result = dict(ledger)
    result[LEDGER_FINGERPRINT_FIELD] = ledger_fingerprint(result)
    return result
...
def commit_action(
    ...
    selected_edge: list[str] | None = None,
) -> dict[str, Any]:
    result = dict(ledger)
    result["last_selected_action"] = action
    result["last_selected_subgoal_id"] = subgoal_id
    result["last_selected_skill_id"] = selected_skill_id
    result["last_selected_edge"] = list(selected_edge) if selected_edge is not None else None
    result["attempted_actions"] = list(ledger.get("attempted_actions") or []) + [action]
    return seal_ledger(result)
...
def structured_score(command: str, pending: dict[str, Any] | None) -> tuple[int, int, list[str]]:
    ...
    kind = pending["kind"]
    if kind == "search_source":
        # Source receptacle is unknown until it appears in observation;
        # exploration actions therefore tie and use admissible order.
        primary = 2 if family == "open" else 1 if family == "goto" else 0
...
def candidate_rows(index: base.SkillIndex, admissible: list[str]) -> tuple[dict[str, list[str]], list[str], list[tuple[int, str, str]]]:
    command_candidates = {command: matching_skill_ids(index, command) for command in admissible}
...
def _rank_selection(
    ...
    """Compute the deterministic selector decision from observable inputs.

    This helper is intentionally side-effect free and is also used by the
    validator. Keeping ranking and trace construction in one path prevents a
    validator from silently accepting a trace whose decision fields were
    ...
    previous_skill = (
        previous_skills[-1]
        if previous_skills
        else observed_ledger.get("last_selected_skill_id")
    )
    previous_edge = (
        previous_edges[-1]
        if previous_edges
        else observed_ledger.get("last_selected_edge")
    )
    had_previous_skill = bool(previous_skills) or observed_ledger.get("last_selected_action") is not None
    had_previous_edge = bool(previous_edges) or observed_ledger.get("last_selected_action") is not None
    return {
        ...
        "skill_changed": had_previous_skill and selected_skill != previous_skill,
        "edge_changed": had_previous_edge and selected_edge != previous_edge,
    }
...
def _build_transition(
    ...
    if previous_ledger is not None:
        stored_fingerprint = previous_ledger.get(LEDGER_FINGERPRINT_FIELD)
        if stored_fingerprint != ledger_fingerprint(previous_ledger):
    ...
    return {
        ...
        "eligibility_rejections": [
            ...
            for command, ids in selection["command_candidates"].items() if not ids
        ],
        ...
        "action_state_fingerprint": action_state_fingerprint(snapshot, selection["terminal"]),
...
def select_condition(
    ...
    valid, message = validate_selector_transition(transition, previous_ledger, index=index)
    if not valid:
...
def validate_selector_transition(
    ...
    *,
    ...
    """Validate a selector trace by recomputing the complete decision.

    Fingerprints protect the observable input payload, but they do not by
    themselves prove that candidate skills, ranking, ledger updates, or edge
    fields were produced by the selector.  A frozen train-only ``SkillIndex``
    is therefore required for validation; the output is rebuilt locally and
    all deterministic decision fields are compared against that rebuild.
    """
    ...
    if not isinstance(runtime.get("historical_actions"), list) or not isinstance(runtime.get("admissible_actions"), list):
        return False, "runtime input types invalid"
    ...
    if ledger.get(LEDGER_FINGERPRINT_FIELD) != ledger_fingerprint(ledger):
    ...
    # Older snapshot history is not recoverable from the three runtime fields;
    # nevertheless, immediate repeats are bound to the previous ledger and
    # cannot be reported as novel progress.
