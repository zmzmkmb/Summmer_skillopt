#!/usr/bin/env python3
"""Run synthetic zero-network audit cases for the TG8 selector v3 contract."""
...
from scripts.run_acl2027_tracegraph_tg8_observable_subgoal_selector_v3 import (  # noqa: E402
    seal_ledger,
    select_condition,
    validate_selector_transition,
)

CONFIG = ROOT / "configs/acl2027/tracegraph_tg8_observable_subgoal_selector_v3.json"
OUTPUT_DIR = ROOT / "artifacts/acl2027_tracegraph_tg8_observable_subgoal_selector_audit_v3"
...
def main() -> int:
    ...
    def tamper_rejection() -> dict[str, Any]:
        row = select_condition(index, "lexical_greedy", runtime(initial_obs, [], ["go to fridge", "go to countertop"]))
        ...
        tampered = copy.deepcopy(row)
        tampered["candidate_skill_ids"] = list(reversed(tampered["candidate_skill_ids"]))
        assert validate_selector_transition(tampered, index=index) == (False, "candidate_skill_ids mismatch")
        tampered = copy.deepcopy(row)
        tampered["selection_reason"] = "forged_reason"
        assert validate_selector_transition(tampered, index=index) == (False, "selection_reason mismatch")
        tampered = copy.deepcopy(row)
        tampered["subgoal_ledger"]["completed_subgoal_ids"] = ["put"]
        assert validate_selector_transition(tampered, index=index) == (False, "ledger fingerprint mismatch")
        return {"tamper_cases": 5}
    ...
    result = {
        "schema_version": 3,
        "phase_id": "TG8-tracegraph-observable-subgoal-selector-v3",
    ...
    audit_path = output_dir / "selector_audit.json"
    ...
    print(json.dumps({"status": result["status"], "cases": len(cases), "passed": result["passed_case_count"], "audit_sha256": sha256(audit_path)}, ensure_ascii=False))
