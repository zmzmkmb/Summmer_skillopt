#!/usr/bin/env python3
"""Run the ACL 2027 Phase 1B SearchQA authorized Token Plan pilot.
...
DEFAULT_CONFIG = (
    PROJECT_ROOT / "configs" / "acl2027" / "searchqa_phase1b_token_plan_live_pilot_v3.json"
)
...
def validate_config(config_path: Path = DEFAULT_CONFIG, root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    _require(execution.get("paid_api_allowed") is True, "v3 config must allow one bounded authorized Token Plan smoke")
    ...
    _require(execution.get("billing_route") == "token_plan_authorized", "v3 runner requires the authorized Token Plan route")
    _require(
