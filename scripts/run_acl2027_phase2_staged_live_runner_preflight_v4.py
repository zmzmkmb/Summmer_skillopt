#!/usr/bin/env python3
"""Phase 2 v4 integrity-rooted closed preflight and staged runner.
...
from scripts.acl2027_phase2_integrity_v4 import IntegrityError, verify_integrity
...
CONFIG = ROOT / "configs/acl2027/phase2_staged_live_runner_preflight_v4.json"
AUTH = ROOT / "configs/acl2027/phase2_staged_live_runner_authorization_closed_v4.json"
MANIFEST = ROOT / "artifacts/acl2027_phase2_staged_live_runner_preflight_v4/run_manifest.json"
...
def validate_config(config: dict[str, Any]) -> None:
    try:
        disk_config, _ = verify_integrity(CONFIG, MANIFEST, root=ROOT)
    except IntegrityError as exc:
        raise HardStop(f"v4 integrity gate failed: {exc}") from exc
    ...
    if any(config["execution"].get(k) is not False for k in ("network_calls_allowed", "provider_calls_allowed", "paid_api_allowed", "qwen_authorization_open", "formal_scaling_allowed")):
        raise HardStop("v4 execution switches must remain closed")
...
def validate_authorization(config: dict[str, Any], auth: dict[str, Any], auth_sha: str, stage: str | None = None) -> None:
    ...
    required_bindings = {
        ...
        "manifest_aggregate_fingerprint": manifest["aggregate_fingerprint"],
