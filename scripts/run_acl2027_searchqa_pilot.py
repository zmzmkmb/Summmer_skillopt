#!/usr/bin/env python3
"""Run the ACL 2027 Phase 1B SearchQA PAYG pilot.

Dry-run and live execution share prompt rendering, retrieval, guard, resume,
and accounting paths. Live mode is additionally gated by the frozen config and
...
DEFAULT_CONFIG = (
    PROJECT_ROOT / "configs" / "acl2027" / "searchqa_phase1b_live_pilot_v2.json"
)
...
def validate_config(config_path: Path = DEFAULT_CONFIG, root: Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    _require(execution.get("paid_api_allowed") is True, "v2 config must allow bounded PAYG")
    _require(execution.get("formal_scaling_allowed") is False, "formal scaling must stay closed")
    _require(execution.get("billing_route") == "pay_as_you_go", "automated runner must use PAYG")
    _require(
        execution.get("hidden_backend_retries_allowed") is False,
        "hidden backend retries must be disabled",
    )
    model = config.get("model", {})
    _require(model.get("model_id") == "qwen3.5-flash", "frozen model must be qwen3.5-flash")
...
class DashScopePaygProvider:
    ...
    def invoke(self, messages: list[dict[str, Any]]) -> LiveAttempt:
        self.invocations += 1
        try:
...
def _conservative_attempt_reservation(config: dict[str, Any]) -> dict[str, Any]:
    return _attempt_record(
        0,
        LiveAttempt(
            status="failed",
            response="",
            input_tokens=int(config["model"]["max_input_tokens_per_attempt"]),
            output_tokens=int(config["model"]["max_output_tokens_per_attempt"]),
    ...
    *,
    ...
    record = {
        **call,
        ...
        "status": "completed",
        "provider": (
            "dashscope_beijing_payg" if mode == "live" else "deterministic_no_network"
        ),
        "paid_call": mode == "live",
        "attempts": attempts,
        "provider_attempts": len(attempts),
...
def execute_prepared_call(
    ...
    *,
    ...
    """Execute or resume one call while journaling every paid attempt."""
    attempts = existing_attempts
    if attempts and attempts[-1]["status"] == "success":
        return _record_from_prepared_attempts(
            call, item, prepared, attempts, mode=mode
        )
    max_attempts = int(config["model"]["max_attempts_per_logical_call"])
    ...
    for attempt_index in range(len(attempts) + 1, max_attempts + 1):
        ...
        reservation = _conservative_attempt_reservation(config)
        _check_attempt_budget(
            ledger,
            config,
            reservation,
        ...
        if mode == "live":
            result = provider.invoke(prepared["messages"])
        else:
            input_tokens = max(
                1,
                (
        ...
        if (
            ...
            > int(config["model"]["max_input_tokens_per_attempt"])
            or int(attempt["output_tokens"])
            > int(config["model"]["max_output_tokens_per_attempt"])
        ):
            raise ProviderUsageError(
    ...
    *,
    ...
    summary = {
        ...
        "gates": {
            ...
            "payg_only": config["execution"]["billing_route"] == "pay_as_you_go",
