#!/usr/bin/env python3
"""Analyze the complete Phase 3D specificity and abstention live grid."""
...
VERSION = 3
PREFLIGHT = ROOT / "artifacts/acl2027_phase3d_specificity_abstention_preflight_v1"
...
LIVE = ROOT / "artifacts/acl2027_phase3d_specificity_abstention_live_v3"
LEDGER = LIVE / "ledger.json"
RUN_AUDIT = LIVE / "run_audit.json"
AUDIT = LIVE / "specificity_analysis.json"
REPORT = ROOT / "paper/acl2027/results/phase3d_specificity_abstention_live_v3.md"
...
def parse_contract(raw: Any, row: dict[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
    ...
    if not isinstance(assessments, list) or len(assessments) != len(order):
        return None, "assessment_length"
    for expected_id, item in zip(order, assessments):
        if not isinstance(item, dict) or set(item) != {"skill_id", "applicable"}:
            return None, "assessment_shape"
        if item["skill_id"] != expected_id or not isinstance(item["applicable"], bool):
...
def analyze() -> dict[str, Any]:
    ...
    if run.get("status") != "completed" or run.get("authorization_closed") is not True or len(ledger) != 240:
        raise RuntimeError("complete closed Phase 3D v3 execution required")
    ...
    if len(by_gold) != 40 or len(by_plan) != 240:
        raise RuntimeError("Phase 3D source grid drift")
    ...
    for record in ledger:
        ...
        if planned is None or task is None or record["request_hash"] != planned["request_hash"]:
            raise RuntimeError("Phase 3D ledger binding drift")
    ...
    if len(grids) != 40 or any(set(rows) != {"cold", "global_only", "contextual_single", "irrelevant_single", "dual_contextual_first", "dual_irrelevant_first"} for rows in grids.values()):
        raise RuntimeError("Phase 3D completed grid is incomplete")
...
def report_text(result: dict[str, Any]) -> str:
    m = result["metrics"]
    return f"""# ACL 2027 Phase 3D specificity and abstention live v3
