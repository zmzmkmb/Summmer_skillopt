...
from scripts.audit_acl2027_phase2_calibration_v6 import close_authorization
...
AUTH = ROOT / "configs/acl2027/phase2_calibration_live_authorization_v6.json"
...
AUDIT_SOURCE = ROOT / "scripts/audit_acl2027_phase2_calibration_v6.py"
...
def validate_live_authorization(registry_path: Path, authorization_id: str) -> dict[str, Any]:
    ...
    if authorization_id not in registry:
        raise HardStop("v6 live authorization missing or closed")
    ...
    if auth.get("live_execution_bindings") != expected:
        raise HardStop("v6 live execution source binding drift")
    if auth.get("user_authorization", {}).get("scope") != "phase2_v6_calibration_only":
        raise HardStop("v6 user authorization scope drift")
    if auth.get("user_authorization", {}).get("not_inherited_by_later_stages") is not True:
        raise HardStop("v6 later-stage prohibition drift")
    return auth
...
class PersistentRequestStartPacer:
    ...
    def __call__(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(starts) != len(records):
            raise HardStop("v6 ambiguous started request; automatic retry forbidden", records)
...
def execute_live(authorization_id: str) -> dict[str, Any]:
    if CLOSURE.exists():
        raise HardStop("v6 live authorization already closed")
