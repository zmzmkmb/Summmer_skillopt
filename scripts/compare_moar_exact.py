...
def exact_select_both(
    ...
    """穷举所有 2^n 组合，返回 (最优子集按 M_exact, M_exact, M_moar)。
    ...
    best_m_exact = -np.inf
    best_subset_exact: list[int] = []
    best_m_moar_for_exact = 0.0
    ...
    for k in range(1, top_k + 1):
        for combo in combinations(range(n), k):
            ...
            if total_cost > budget:
                continue
            m = compute_m_exact(combo_list, relevance, utilities,
                               token_costs, pairwise_sims, top_k, budget, weights)
            if m > best_m_exact:
                best_m_exact = m
                best_subset_exact = combo_list

    # 计算该最优子集在 MOAR 指标下的分数
    if best_subset_exact:
        best_m_moar_for_exact = compute_m_moar(
            best_subset_exact, relevance, utilities,
            token_costs, pairwise_sims, top_k, budget, weights,
        )

    return best_subset_exact, best_m_exact, best_m_moar_for_exact
...
def main():
    ...
    exact_hits = 0        # MOAR 选中了与 Exact 完全相同的规则集（M_exact 度量）
    moar_better = 0       # MOAR 的 M 值超过 Exact（理论上极少 / 度量不一致）
    gaps_exact: list[float] = []  # M_exact(Exact) - M_exact(MOAR)
    gaps_moar: list[float] = []   # M_moar(MOAR) - M_moar(Exact)，MOAR 自我度量下是否更优
    jaccards: list[float] = []
    ...
    for qi, item in enumerate(items):
        ...
        exact_indices, exact_m_exact, exact_m_moar = exact_select_both(
        ...
        # ── 比较（双重度量） ─────────────────────────────────────────────
        gap_exact = exact_m_exact - moar_m_exact   # ≥0：MOAR 低于精确最优
        gap_moar = moar_m_moar - exact_m_moar       # ≥0：Exact 在 MOAR 度量下低于 MOAR
        if abs(gap_exact) < 1e-10:
            exact_hits += 1
        elif gap_exact < 0:
            moar_better += 1
        gaps_exact.append(gap_exact)
        gaps_moar.append(gap_moar)

        # Jaccard
        set_moar = set(moar_indices)
        set_exact = set(exact_indices)
        if not set_moar and not set_exact:
            jac = 1.0
        elif not set_moar or not set_exact:
            jac = 0.0
        else:
            jac = len(set_moar & set_exact) / len(set_moar | set_exact)
        jaccards.append(jac)
        ...
        per_query.append({
            ...
            "exact_m_exact": float(exact_m_exact),
            "exact_m_moar": float(exact_m_moar),
            "gap_exact": float(gap_exact),
            "gap_moar": float(gap_moar),
            "jaccard": float(jac),
        ...
        if (qi + 1) % 50 == 0:
            cur_gaps = np.array(gaps_exact)
            cur_gaps_m = np.array(gaps_moar)
            cur_jacs = np.array(jaccards)
            print(f"  [{qi+1}/{n_queries}] "
                  f"hit(M_exact)={exact_hits/(qi+1)*100:.0f}% "
                  f"gap_exact_mean={np.mean(cur_gaps):.4f} "
                  f"gap_moar_mean={np.mean(cur_gaps_m):.4f} "
                  f"J_mean={np.mean(cur_jacs):.3f} "
                  f"moar={np.mean(moar_times):.0f}ms/q "
                  f"exact={np.mean(exact_times):.0f}ms/q")
    ...
    hit_rate = exact_hits / n_queries
    jac_mean = float(np.mean(jaccards_arr))
    ...
    print(f"")
    print(f"  === 指标 A：统一 M_exact 度量（与 greedy/exact 一致） ===")
    print(f"  {'指标':<35s} {'值':>15s}")
    print(f"  {'-'*50}")
    print(f"  {'MOAR 命中精确最优解比例':<35s} {hit_rate:14.1%}")
    print(f"  {'平均差距 (M_exact - M_moar)':<35s} {float(np.mean(gaps_exact_arr)):14.4f}")
    print(f"  {'差距中位数':<35s} {float(np.median(gaps_exact_arr)):14.4f}")
    print(f"  {'差距 P95':<35s} {float(np.percentile(gaps_exact_arr, 95)):14.4f}")
    print(f"  {'差距最大值':<35s} {float(np.max(gaps_exact_arr)):14.4f}")
    print(f"  {'平均 Jaccard':<35s} {jac_mean:14.4f}")
    print(f"")

    # 在 MOAR 自身度量下：MOAR 是否"超过" Exact？
    # gap_moar = M_moar(MOAR) - M_moar(Exact)，正值表示 MOAR 在自身度量下更优
    moar_wins_own = int(np.sum(gaps_moar_arr > 1e-10))
    moar_ties_own = int(np.sum(np.abs(gaps_moar_arr) < 1e-10))
    exact_wins_own = int(np.sum(gaps_moar_arr < -1e-10))

    print(f"  === 指标 B：MOAR 自身度量（NSGA-II 内部目标函数） ===")
    print(f"  说明: gap_moar = M_moar(MOAR) - M_moar(Exact)")
    print(f"        正值 = MOAR 在自身度量下更优")
    print(f"        负值 = Exact 在 MOAR 度量下更优（MOAR 未收敛到 Pareto 前沿）")
    print(f"  {'指标':<35s} {'值':>15s}")
    print(f"  {'-'*50}")
    print(f"  {'MOAR 在自身度量下更优':<35s} {moar_wins_own:>14d} 条 ({moar_wins_own/n_queries*100:5.1f}%)")
    print(f"  {'双方相等':<35s} {moar_ties_own:>14d} 条 ({moar_ties_own/n_queries*100:5.1f}%)")
    print(f"  {'Exact 在 MOAR 度量下更优':<35s} {exact_wins_own:>14d} 条 ({exact_wins_own/n_queries*100:5.1f}%)")
    print(f"  {'MOAR 自身度量平均差距':<35s} {float(np.mean(gaps_moar_arr)):14.4f}")
    print(f"")
    print(f"  检索延迟:")
    print(f"    MOAR  mean={np.mean(moar_t_arr):.0f}ms "
          f"median={np.median(moar_t_arr):.0f}ms "
          f"P95={np.percentile(moar_t_arr, 95):.0f}ms")
    print(f"    Exact mean={np.mean(exact_t_arr):.0f}ms "
    ...
    print(f"\n  {'='*60}")
    print(f"  结论:")
    if exact_wins_own == 0 and moar_wins_own >= 0:
        print(f"    [OK] MOAR 在所有 query 上达到了自身 NSGA-II 目标函数的 Pareto 最优。")
        print(f"       两者在 M_exact 度量下的差异源于归一化方式不同")
        print(f"       （MOAR 用 max_rel 归一化，Exact 用 top_k 归一化），")
        print(f"       这是算法设计选择，而非搜索缺陷。")
    else:
        print(f"    [WARN] MOAR 在 {exact_wins_own} 条 query 上未达到自身目标函数的 Pareto 最优，")
        print(f"       可能需要增加 pop_size 或 generations。")
    print(f"  {'='*60}")
    ...
    summary = {
        ...
        "results": {
            "n_queries": n_queries,
            "metric_exact": {
                "description": "统一 M_exact 度量（除以 top_k 归一化）",
            ...
            "metric_moar": {
                "description": "MOAR 自身度量（除以 max_rel 归一化）",
                "moar_better": moar_wins_own,
                "tie": moar_ties_own,
                "exact_better": exact_wins_own,
