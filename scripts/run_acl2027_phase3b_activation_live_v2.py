...
AUTH_ID = "phase3b-activation-live-v2"
RECEIPT = ROOT / "configs/acl2027/phase3b_activation_user_authorization_receipt_v2.json"
...
ANALYZER = ROOT / "scripts/analyze_acl2027_phase3b_activation_live_v2.py"
TEST = ROOT / "tests/test_acl2027_phase3b_activation_live_v2.py"
ARTIFACT = ROOT / "artifacts/acl2027_phase3b_activation_live_v2"
...
def preflight() -> dict[str, Any]:
    ...
    if receipt.get("status") != "explicit_user_authorization_received_execution_not_open" or receipt.get("phase3b_aggregate_fingerprint") != fingerprint or any(receipt.get(k) != v for k, v in exact_contract().items()): raise HardStop("Phase 3B v2 receipt boundary drift")
    ...
    if receipt.get("bindings") != expected: raise HardStop("Phase 3B v2 receipt binding drift")
    if any(receipt.get("execution", {}).get(k) is not False for k in ("network_calls_allowed", "provider_calls_allowed", "paid_api_allowed", "qwen_authorization_open", "formal_scaling_allowed")): raise HardStop("Phase 3B v2 receipt is not execution-closed")
...
def authorization_template(key: str) -> dict[str, Any]:
    gate = preflight()
    return {"schema_version": VERSION, "experiment": "acl2027_phase3b_activation_live_v2", "authorization_id": AUTH_ID, "status": "open", "bindings": {**gate["bindings"], "zero_network_preflight_aggregate_fingerprint": stable(gate)}, "credential_sha256": hashlib.sha256(key.encode()).hexdigest(), **exact_contract(), "endpoint": "https://token-plan.cn-beijing.maas.aliyuncs.com/compatible-mode/v1", "request_interval_seconds": 1.0, "paid_api_allowed": True, "provider_calls_allowed": True, "qwen_authorization_open": True, "formal_scaling_allowed": False, "forbidden_stages": load(RECEIPT)["forbidden_stages"]}


def open_authorization() -> dict[str, Any]:
    key = os.environ.get("DASHSCOPE_API_KEY")
    if not key: raise HardStop("DASHSCOPE_API_KEY missing")
    if any(path.exists() for path in (AUTH, AUTH_CLOSED, REGISTRY, LEDGER, PACING, CLOSURE)): raise HardStop("Phase 3B v2 authorization already opened, started, or closed")
...
def validate_authorization() -> tuple[dict[str, Any], str]:
    key = os.environ.get("DASHSCOPE_API_KEY")
    if not key or not AUTH.is_file() or not REGISTRY.is_file() or AUTH_CLOSED.exists() or CLOSURE.exists(): raise HardStop("Phase 3B v2 authorization is closed or absent")
    auth = load(AUTH)
    if auth != authorization_template(key): raise HardStop("Phase 3B v2 authorization drift")
    item = load(REGISTRY)["authorizations"].get(AUTH_ID)
    if not item or item.get("sha256") != sha256_file(AUTH): raise HardStop("Phase 3B v2 authorization registry drift")
